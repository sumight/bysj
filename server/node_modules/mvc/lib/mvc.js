...
function init(_appDirname){
    ...
    // 支持client
    nullControllers = JSON.parse(JSON.stringify(controllers));
    initNullControllers(nullControllers);
    controllers.getControllers = function(){
        return data(nullControllers);
    }
}

/**
* 初始化nullControllers
* @function initNullControllers
* @param {Object} nullControllers
*/
function initNullControllers(nullControllers){
    console.log(JSON.stringify(controllers));
    for(key in nullControllers){
        if(typeof nullControllers[key] == 'function'){
            nullControllers[key] == null;
        }else{
            initNullControllers(nullControllers[key]);
        }
    }
}
...
function responseSourse(req, res) {
    ...
    /* global appDirname */ 
    var filepath = path.join(appDirname, 'source');
    /* global pathname */
...
function view(data){
    /* global appDirname */ 
    var filepath = path.join(appDirname, 'views');
    var filepath = path.join(filepath, pathname);
    filepath += '.ejs';
