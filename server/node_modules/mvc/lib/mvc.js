var http = require('http'),
    url = require('url'),
    path = require('path'),
    fs = require('fs'),
    ejs = require('ejs'),
    assert = require('assert'),
    os = require('os'),
    mineObj = require('./mine.json'),
    cookie = require('simple-cookie'),
    ObjectID = require('mongodb').ObjectID
/** 
* 所有的控制器都注册在这里
* @global controllers
*/ 
var controllers = {};

/**
* 应用所在的位置，在mvc调用init的时候确定
* @global appDirname
*/ 
var appDirname = ''

/**
* 用来存储所有用户会话信息，以键值对的形式，key是sessionid，value是session对象
* @global sessionStorage
*/
var sessionStorage = {};

/**
* 用来存储当前用户会话信息，以键值对的形式，key是sessionid，value是session对象
* @global session
*/
var session = null;

/**
* 用来记录req对象
* @global request
*/
var request = null;
/**
* 用来记录res对象
* @global response
*/
var response = null;

var server = http.createServer(function(req,res){
    req.pathname = url.parse(req.url).pathname;
    req.query = url.parse(req.url,true).query;
    req.setEncoding('utf8');
    req.on("data",function(chunk){
        if(req.body === undefined)
            req.body = ""
        req.body += chunk
    })
    req.on('end',function(){
        // 解析json对象
        if(!!req.body){
            req.body = JSON.parse(req.body);
            for(key in req.body){
                if(key.match(/^_id/)){
                    // 将ObjectID字符串传化成对应的对象
                    req.body[key] = new ObjectID(req.body[key])
                }
                if(key.match(/Date$/)){
                    // 将日期字符串转化成对应的对象
                    req.body[key] = new Date(req.body[key]);
                }
            }
        }

        var responseData = null;

        if(!isControllerRequest(req)){
            responseSourse(req,res);
        }else{
            var params = extractParams(req) || {};
            var names = req.pathname.split('/');
            names.shift();
            var method = controllers;
            for(key in names){
                var name = names[key];
                method = method[name];
                if(!method){
                    res.end('not found');
                    return ;
                }
            }
            var flag = setInterval(function(){
                if(response === null){
                    /* global request */
                    request = req; 
                    /* global response */
                    response = res;
                    method(params);
                    clearInterval(flag);
                }
            },1)
            
        }
    })
})


/**
* 初始化mvc
* @function init
* @param appDirname
*/
function init(_appDirname){
    /* global appDirname */ 
    appDirname = _appDirname;
    loadControllers(path.join(_appDirname, 'controllers'));
}


/**
* 判断是否为控制器请求
* @function isControllerRequest
* @param {Request} req 请求对象
* @returns {Boolean} 
*/
function isControllerRequest(req){
    var pathname = url.parse(req.url).pathname;
    var hasSuffix = /\.[a-z]+$/.test(pathname);
    if(hasSuffix)
        return false;
    return true;
}

/**
 * 获取一个文本文件
 * @function getPage
 * @param {HttpRequest} req 响应对象
 * @param {HttpResponse} res 响应对象
 */
function responseSourse(req, res) {
    var pathname = req.pathname;
    /* global appDirname */
    var filepath = path.join(appDirname, 'source', pathname);
    fs.exists(filepath, function(exists) {
        if (exists) {
            fs.readFile(filepath, function(err, data) {
                assert.equal(null, err)
                suffix = filepath.replace(/.*\./,'');
                res.writeHeader(200, {'Content-Type': mineObj[suffix]+';charset=utf-8'});
                res.end(data);
            })
        } else {
            res.writeHeader(404, {})
            res.end("未找到资源")
        }
    })
}

/**
* 提取参数
* @function extractParams
* @param {Request} req http请求对象
* @returns {Object} 提取的参数
*/
function extractParams(req){
    var params = {};
    if(req.method === 'POST'){
        params[0] = req.body;
    }
    if(req.method === 'GET'){
        params = req.query;
    }
    return params;
}

/**
* 生成一个视图
* @function view
* @param {Object} data 数据
* @returns {String} html字符串
*/
function view(data){
    /* global appDirname */ /* global request */
    var filepath = path.join(appDirname, 'views', request.pathname);
    filepath += '.ejs';
    if(fs.existsSync(filepath)){
        var fileData = fs.readFileSync(filepath).toString();
        /* global response */
        response.end(ejs.render(fileData,data));
        /* global response */ // 收回response全局变量
        response = null;
    }
}

/**
* 生成一个数据
* @function data
* @param {Object} data 数据
* @returns {String} 序列化的数据
*/

function data(data){
    /* global response */
    response.end(JSON.stringify(data))
    /* global response */ // 收回response全局变量
    response = null;
}

/**
* 加载controllers路径下的所有控制器
* @function loadControllers
* @param {String} ctrlsPath 控制器的路径
* @returns {Object} 控制器对象
*/
function loadControllers(ctrlsPath){
    var itemArray = fs.readdirSync(ctrlsPath);
    for(var key in itemArray){
        var stats = fs.statSync(path.join(ctrlsPath,itemArray[key]));
        var itemPathname = path.join(ctrlsPath,itemArray[key]);
        if(stats.isFile()){
            if(!!os.type().match(/^win/i))
                var objectPath = itemPathname.replace(/^.*\\controllers(?=\\)/,"").replace(/\.js$/,'');
            else
                var objectPath = itemPathname.replace(/^.*\/controllers(?=\/)/,"").replace(/\.js$/,'');
            var controller = require(itemPathname);
            addValueByPath(controllers, objectPath, controller);
        }
        if(stats.isDirectory()){
            loadControllers(itemPathname);
        }
    }
}

/**
* 用路径访问js对象
* @function addValueByPath
* @param {Object} obj 被访问的对象
* @param {String} pathname 路径
* @param {Object} value 赋值给目标对象的对象
* @returns {Object} 访问以后的结果
*/
function addValueByPath(obj, pathname, value){
    if(!!os.type().match(/^win/i))
        var names = pathname.split('\\');
    else
        var names = pathname.split('/');
    names.shift();
    var subObj = obj;
    for(key in names){
        var name = names[key];
        if(subObj[name] === undefined){
            subObj[name] = {}
        }
        if(key == (names.length-1)){
            subObj[name] = value;
        }
        subObj = subObj[name];
    }
    return obj;
}

/**
* 获取session，如果session没有存在，则先创建什么
* @function getSession
*/
function getSession(){
    // 判断请求cookie中是否存在sessionid
    /* global request */
    if(request.headers['cookie']){
        /* global request */
        var cookieObject = cookie.parse(request.headers['cookie']);
        if(cookieObject.name === 'sessionid'){
            if(sessionStorage[cookieObject.value]!==undefined){
                // 如果session已经存在
                return sessionStorage[cookieObject.value];
            }
        }
    }
    var sessionid = new ObjectID().toString();
    var cookieObject = {
        name: 'sessionid',
        value: sessionid,
        expires: (new Date()).valueOf() + 1000*3600*24*10,
        path: '/',
        httponly: false,
        secure: false
    }
    /* global response */
    response.setHeader('Set-Cookie',cookie.stringify(cookieObject));
    sessionStorage[sessionid] = {};
    return sessionStorage[sessionid];
}
/**
* 获取一个新的session，覆盖之前的session
* @function getNewSession
*/
function getNewSession(){
    var sessionid = new ObjectID().toString();
    var cookieObject = {
        name: 'sessionid',
        value: sessionid,
        expires: (new Date()).valueOf() + 1000*3600*24*10,
        path: '/',
        httponly: false,
        secure: false
    }
    /* global response */
    response.setHeader('Set-Cookie',cookie.stringify(cookieObject));
    sessionStorage[sessionid] = {};
    return sessionStorage[sessionid];
}

module.exports = {
    controllers:controllers,
    view:view,
    data:data,
    server:server,
    init:init,
    getSession:getSession,
    getNewSession:getNewSession
}

